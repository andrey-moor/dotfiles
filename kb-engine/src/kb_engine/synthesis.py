...
def synthesis_candidates(
    ...
    (filename match on the slug — keep it simple). The result is sorted by member
    count descending so the richest synthesis targets surface first.
    """
    ...
    candidates = [
        ...
        and not (wiki_dir / f"{topic.slug}.md").exists()
    ]
