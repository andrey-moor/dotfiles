# modules/home/linux/edge-rosetta.nix -- Microsoft Edge Browser (aarch64/Rosetta)
#
# Installs Microsoft Edge browser on aarch64-linux via Rosetta emulation.
# Uses x86_64 Mesa for software rendering, bubblewrap for NixOS-compatible
# /run/opengl-driver paths, and SIGTRAP workaround for crashpad.
#
...
with lib;
let
  cfg = config.modules.linux.edge-rosetta;
...
in {
  options.modules.linux.edge-rosetta = {
    enable = mkEnableOption "Microsoft Edge browser (aarch64/Rosetta)";
  };
