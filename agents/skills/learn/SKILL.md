...
Teaching is not summarizing. A summary compresses the reference; a lesson gives the reader a path: the gist first, one concrete example followed end to end, each part explained with its reason for existing, the traps, and a way to check what stuck. The output is a study Markdown file in a fixed shape, rendered to an interactive single-file page by `scripts/build-study.mjs`.
...
**REQUIRED SUB-SKILL:** doc-diagrams for the mechanism figure, in step mode (see its Step mode section).

Script paths are relative to this skill's directory. First use on a machine: `bash scripts/setup.sh`. A skill created or installed during the current session may not be listed by the Skill tool until after the next tool call. If a named skill is missing, retry once, then read its SKILL.md directly.

## Workflow

1. **State the plan in one message:** topic, goal (interview preparation, design review, operating it, curiosity), sources, depth, and the one worked example you will thread through. Proceed unless redirected.
2. **Get notes.** Use the topic's `notes/` folder when `python3 ~/.claude/skills/research-notes/scripts/check-notes.py notes/ --sources <cited folder>` passes. Otherwise run research-notes first. A dense existing document counts as a source: research-notes distills it into tagged claims before you teach from it.
3. **Pick the worked example.** One concrete instance (one request, one job, one packet, one deploy) with fixed values that touches every part. The whole lesson follows it. Arithmetic on its values lives in the study file, never in the notes.
...
6. **Build:** `node scripts/build-study.mjs study.md` writes `study.html`. It stops on a broken rule and warns on a weak one (see Builder checks). Fix the warnings before delivering.
...
| `# Worked example` | yes | A lead that declares the example's values as illustrative: one sentence, then a short list when there are more than three values. Then `## Step N: title` subsections numbered from 1 (or the `walkthrough` word instead of Step), each two to four sentences following the example. The page turns them into a stepper that moves with the figure |
...
- **One name per concept.** If the system has steps of its own (engine steps, training steps), keep the word for the system and set `walkthrough: Stage` for the lesson. If a common word appears in the lesson in two senses, qualify the technical one everywhere (KV block) and put the qualified phrase in the glossary.
...
A later lesson starts where the previous one ended. It reuses the earlier example values and declares any new assumption as new in its lead. Each lesson's Go deeper starts with links to the lessons before and after it.
