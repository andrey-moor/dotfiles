...
### Task 1.3: Force the output via Limine kernel cmdline + reboot

**Files:** `/boot/limine.conf` (system)

This makes `HDMI-A-1` stay enabled with the dumped EDID even when the Dell is off/unplugged — the permanent fix for the wayvnc "output went away" failure.

- [ ] **Step 1 (🧑 USER): Back up limine.conf**

Run: `sudo cp /boot/limine.conf /boot/limine.conf.bak-$(date +%Y%m%d)`

- [ ] **Step 2 (🧑 USER): Append the two kernel params to the active boot entry's `cmdline`**

Edit `/boot/limine.conf` (e.g. `sudo $EDITOR /boot/limine.conf`). On the active entry's `cmdline=` line, append (space-separated):
```
drm.edid_firmware=HDMI-A-1:edid/rocinante-stream.bin video=HDMI-A-1:e
```
(`:e` forces the connector enabled.) Save.

- [ ] **Step 3 (🧑 USER): Reboot**

Run: `sudo reboot`  — then reconnect (`ssh rocinante`) and resume the plan.

- [ ] **Step 4 (🤖 AGENT): Verify the forced EDID is active**
...
### Task 1.4: SDDM autologin (session always present on boot)

**Files:** `/etc/sddm.conf.d/10-autologin.conf` (system)

- [ ] **Step 1 (🤖 AGENT): Find the exact Hyprland session name**

Run: `ls /usr/share/wayland-sessions/`
Expected: a `hyprland.desktop` (note the exact filename for the next step).

- [ ] **Step 2 (🧑 USER): Write the autologin drop-in**

Run in your terminal (adjust `Session=` if the filename differs):
```
sudo install -d /etc/sddm.conf.d
printf '[Autologin]\nUser=andreym\nSession=hyprland.desktop\n' | sudo tee /etc/sddm.conf.d/10-autologin.conf
```

- [ ] **Step 3 (🧑 USER): Reboot and confirm it auto-enters Hyprland without a password prompt**

Run: `sudo reboot`, observe local console (or just confirm via Step 4 after reconnecting).

- [ ] **Step 4 (🤖 AGENT): Verify a Hyprland session is running for andreym**
