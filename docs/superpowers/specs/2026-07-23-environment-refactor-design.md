...
| **windows-vm** | Parallels on behemoth | Windows 11 (existing, domain-joined) | Sanctioned corporate fallback; not nixed; backed up |
...
| **B — Intune path** | Spike-driven. **Spike B4**: NixOS VM + himmelblau (corp precedent exists; honest distro reporting; watch AADSTS 530003 CA catch-22). **Spike B3**: NixOS VM + intuneme (official agent in Ubuntu nspawn container; passes unmodified tenants; unproven on NixOS). **B2 regardless**: genuine Ubuntu baseline VM. Claude preps VMs to the enrollment doorstep; owner does the credential/MFA ceremony. Success bar: enrolled + Compliant + CA-gated resource opens + survives reboot + tolerable ergonomics. The old `intune.nix` (657 lines) + os-release spoof retire unported. **Fuse: Ubuntu 22.04 exits Intune support Aug 2026 — today's spoofed rocinante claims 22.04; spikes are the top-priority execution item.** |
| **C — Mac-side VM** | Parallels stays (already licensed; hosts the Windows VM). Successor = aarch64 NixOS in Parallels, same dev layer, corp layer = himmelblau-or-nothing (no Rosetta port). Weekly keep-alive: boot → `nh` switch → Intune check-in (if enrolled) → shutdown. Fire drill before old stargazer is deleted. |
