...
<task type="auto">
  ...
  <action>
    ...
    Update hosts/endurance/README.md to ensure complete template creation documentation exists:

    1. **Verify existing documentation covers Plan 01 (LUKS VM creation):**
       - Check if README has VM creation section (create-arch-vm.sh usage)
       - Check if README has LUKS installation section (install-arch.sh usage)
       - If missing, add sections documenting these steps

    2. **Verify existing documentation covers Plan 02 (Omarchy installation):**
       - Check if README has Omarchy installation section (armarchy installer)
       - Check if README documents greetd configuration
       - If missing, add sections documenting these steps

    3. **Add "Template Generalization" section** (if not already present):
       The section should document:
       - When to generalize (before converting to template)
       - What the generalization script does
       - How to convert VM to template
       - How to clone template for new VMs
    ...
    This template was created following these steps (documented for reproducibility):

    ### Step 1: Create Encrypted Base VM

    ```bash
    # Create VM with Parallels (Rosetta, shared folders, bridged network)
    ./scripts/create-arch-vm.sh ArchBase-Template

    # Start VM and open Parallels console
    prlctl start ArchBase-Template

    # Run installation script in VM console (uses vm-type.sh)
    ...
    ### Step 2: Install Omarchy Desktop

    ```bash
    # SSH into VM after reboot
    ssh user@<VM_IP>

    # Update system and install Omarchy
    ...
    ### Step 3: Generalize and Convert to Template
    ...
    Integrate this into the existing README structure, avoiding duplication with any existing sections.
  </action>
  <verify>
    `prlctl list -a | grep TestClone` returns empty (clone deleted)
    hosts/endurance/README.md contains "Template Creation Process" or "Template Generalization" section
    hosts/endurance/README.md documents all three phases (VM creation, Omarchy install, generalization)
    scripts/generalize-template.sh is committed to repo
  </verify>
  <done>Test clone removed and complete documentation updated</done>
</task>
...
grep -q "Template Creation Process\|Template Generalization" ~/Documents/dotfiles/hosts/endurance/README.md
grep -q "create-arch-vm.sh\|install-arch.sh" ~/Documents/dotfiles/hosts/endurance/README.md
grep -q "armarchy\|Omarchy" ~/Documents/dotfiles/hosts/endurance/README.md
...
6. hosts/endurance/README.md documents complete template creation process (Plans 01 + 02 + 03)
</success_criteria>
