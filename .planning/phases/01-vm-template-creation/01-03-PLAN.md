...
<task type="auto">
  ...
  <action>
Add to hosts/endurance/README.md after Omarchy installation (within the VM Template Creation section, before the existing Prerequisites section):
...
4. Continue with Phase 2 (Rosetta/Nix infrastructure) - see [Prerequisites](#prerequisites) below
...
IMPORTANT: Insert this content AFTER the Omarchy installation section (Steps 8-10) but BEFORE the existing Prerequisites section. The existing Phase 2+ content must remain intact. The "Post-Clone Configuration" section should link to the existing Prerequisites section for Phase 2 setup.
  </action>
  <verify>
...
- Post-clone steps documented with link to Prerequisites
- Existing Phase 2+ content (Prerequisites onward) preserved
  </verify>
